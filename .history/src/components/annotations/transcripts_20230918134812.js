...
const Transcripts = (props) => {
    ...
    const handleSelectTranscript = (e) => {
        ...
        console.log('e.target.value',e.target.value,ids)
        let ann = []
        for (let key in ids) {
            console.log('key',key),annotations[parseInt(ids[key]) - 1]
            ann.push(annotations[parseInt(ids[key]) - 1])
        }
        console.log('ann',parseInt(e.target.value) - 1,annotations,ann)
