...
const Transcripts = (props) => {
    ...
    const processTranscripts = (ids,val) => {
        ...
        for (let key in ids) {
            ...
            item.transcript.map( (transcript,i) => { item.transcript[i].text = `${item.transcript[i].text}<div><span class="inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-gray-700 bg-gray-200 rounded">${item.label}</span></div>` } )
