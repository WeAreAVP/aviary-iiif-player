...
const Transcripts = (props) => {
    ...
    const processTranscripts = (ids,val) => {
        ...
        for (let key in ids) {
            ...
            for (let tKey in item.transcript) {
                item.transcript[tKey].text = item.transcript[tKey].text+item.label
            }
            item.transcript.map( (transcript,i) => { item.transcript[i].text = `${item.transcript[i].text}<span>${item.label}</span>` } )
            console.log('item',item.transcript)
