        ...
        <div>
            ...
            <div className="" onMouseOver={() => handleMouseOver(true)} onMouseLeave={() => handleMouseOver(false)}>
                ...
                <div className="custom-select">
                    ...
                    <Select
        defaultValue={annotations ? { value: 1, label: annotations[0].label, color:'#A2849A' }: {}}
        onChange={handleSelectTranscript}
        options={annotations.map((e, key) => {
            let b = _.findIndex(transcriptNames, function(el) { return el[0] == e.label; });

            console.log(b,'transcriptNames',transcriptNames,e.label,indexOf2d(transcriptNames, e.label))
            return { value: key + 1, label: e.label, color:'#A2849A' };
        })}
        isMulti={true}
        styles={selectStyles}
      />
                    ...
                    <div>
                        {transcriptNames.map((point) => {
                            return <span className="inline-flex items-center justify-center px-2 py-1 text-xs font-bold text-gray-700 bg-gray-200 rounded" style={{backgroundColor: point[0]}}>{point[1]}</span>
                        }
                        )}
